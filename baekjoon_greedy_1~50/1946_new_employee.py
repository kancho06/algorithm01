...
#
# 그래서 진영 주식회사는, 다른 모든 지원자와 비교했을 때 서류심사 성적과 면접시험 성적 중 적어도 하나가 다른 지원자보다
...
t = int(input())
for i in range(t):
    count = 1   # 서류 순위 정렬에서 첫번째 사람은 무조건 다른사람보다 성적이 높음으로 합격됨
    p = []

    n = int(input())
    for j in range(n):
        document, interview = map(int, sys.stdin.readline().split())
        p.append([document, interview])

    p.sort()    # 서류 기준 오름차순 정렬
    max = p[0][1]
