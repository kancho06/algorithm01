...
# 우선순위 큐는 작은값이 먼저 나오기 때문에 정렬을 안해도 된다.
import heapq
import sys
...
if len(c) == 1:     # 1개일 경우 비교하지 않아도 된다
    print(0)
else:
    result = 0
    while len(c) > 1:
        temp_1 = heapq.heappop(c)   # 제일 작은 덱
        temp_2 = heapq.heappop(c)   # 두번째로 작은 덱
        result += temp_1 + temp_2   # 현재 비교 횟수를 더해줌
        heapq.heappush(c, temp_1+temp_2)    # 더한 덱을 다시 넣어줌
    print(result)