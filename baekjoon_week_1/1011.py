...
def count(distance): # 목표위치 - 출발위치 = 거리 의 값을 넣어서 몇번 작동해야 최솟값인지 구하는 함수
    i = 0       # 횟수가 들어갈 변수 i
    while i <= distance: # 횟수가 거리와 같거나 적을때 까지 반복 (리턴 될때 까지 반복됨)
        if i * i < distance <= i * i + i:
            return 2 * i  # 최솟값이 짝수일때 횟수
        elif distance <= (i + 1) * (i + 1):
            return i * 2 + 1  # 최솟값이 홀수일때 횟수
        i += 1
